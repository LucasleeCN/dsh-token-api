...
#dsh-client-ui-token-billing-fab:hover {
  background: var(--dsw-alias-button-floating-hover, rgba(255, 255, 255, 0.98));
...
.dsh-tb-fab-reasoning button:hover {
  background: var(--dsw-alias-interactive-bg-hover, rgba(38, 49, 72, 0.08));
  color: var(--dsw-alias-label-primary, #0f172a);
}
.dsh-tb-fab-reasoning button.dsh-tb-active {
...
.dsh-tb-reasoning input[type="range"]::-webkit-slider-thumb:hover { box-shadow: 0 2px 8px rgba(77, 107, 254, 0.4); }
...
`
    ...
    function applyFabHeight(root) {
      ...
      fab.style.height = Math.max(40, Math.min(height, 88)) + "px"
      fab.style.borderRadius = Math.round(Math.max(10, Math.min(height, 88)) * 0.32) + "px"
    }
...
<div id="dsh-client-ui-token-billing-fab" class="dsh-tb-fab" role="button" tabindex="0" aria-label="打开 Token 计费面板">
  ...
  <div class="dsh-tb-fab-info">
    <span class="dsh-tb-fab-label" id="dsh-tb-fab-label">Token</span>
    <span class="dsh-tb-fab-model" id="dsh-tb-fab-model">-</span>
  </div>
  <span class="dsh-tb-fab-divider"></span>
  <div class="dsh-tb-fab-reasoning" id="dsh-tb-fab-reasoning" role="group" aria-label="推理强度"></div>
</div>
<div id="dsh-client-ui-token-billing-panel" class="dsh-tb-hidden">
  ...
  <div class="dsh-tb-foot">
    ...
    function bindEvents(root) {
      var fab = root.querySelector("#dsh-client-ui-token-billing-fab")
      var fabReasoning = root.querySelector("#dsh-tb-fab-reasoning")
      var panel = root.querySelector("#dsh-client-ui-token-billing-panel")
      ...
      if (fabReasoning) {
        fabReasoning.addEventListener("click", function (event) {
          var button = event.target && event.target.closest ? event.target.closest("button") : null
          if (!button) return
          event.preventDefault()
          event.stopPropagation()
          var effortId = button.getAttribute("data-effort-id")
          if (effortId) applyReasoningEffort(effortId)
        })
      }
    ...
    function renderFabReasoning(efforts, activeIndex) {
      ...
      var html = ""
      for (var i = 0; i < efforts.length; i++) {
        html += "<button type=\"button\" data-effort-id=\"" + escapeHtml(efforts[i].id) + "\" class=\"" + (i === activeIndex ? "dsh-tb-active" : "") + "\">" + escapeHtml(efforts[i].name || efforts[i].id) + "</button>"
      }
