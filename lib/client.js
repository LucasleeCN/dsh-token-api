...
#dsh-client-ui-token-billing-fab:active { transform: translateY(0); cursor: grabbing; }
...
`
    ...
    function measureChatWidth() {
      try {
        var viewport = window.innerWidth || document.documentElement.clientWidth || 1200
        var sidebar = document.querySelector('[class*="hHd-Xa_root"]')
        var sidebarWidth = sidebar ? sidebar.getBoundingClientRect().width : Math.min(320, Math.round(viewport * 0.22))
        var width = viewport - sidebarWidth - 24
        return Math.max(320, Math.min(width, 960))
      } catch (error) {
        return 640
      }
    }

    function applyPanelWidth(root) {
      var panel = root.querySelector("#dsh-client-ui-token-billing-panel")
      if (!panel) return
      var width = measureChatWidth()
      panel.style.width = width + "px"
      // 面板宽度与聊天区一致时，向右边缘对齐会覆盖聊天区；保持右边缘与视口一致。
      var fab = root.querySelector("#dsh-client-ui-token-billing-fab")
      if (fab) {
        fab.style.width = Math.max(148, Math.min(220, Math.round(width * 0.42))) + "px"
      }
    }
    ...
    function buildPanel() {
      ...
      root.id = ROOT_ID
      root.innerHTML = `
<button id="dsh-client-ui-token-billing-fab" type="button" aria-label="打开 Token 计费面板">
