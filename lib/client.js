...
`
    ...
    function applyFabHeight(root) {
      var fab = root.querySelector("#dsh-client-ui-token-billing-fab")
      if (!fab) return
      var composerHeight = measureComposerHeight()
      // 以 Harness 输入框高度为基准，再减去 50px（例如 150px -> 100px）。
      var target = Math.max(56, composerHeight - 50)
      fab.style.height = target + "px"
      fab.style.minHeight = target + "px"
      fab.style.overflow = "hidden"
      fab.style.borderRadius = Math.round(target * 0.18) + "px"
    }
  ...
  <div class="dsh-tb-foot">
    ...
    function apply(ctx) {
      ...
      var onDocumentPointerDown = function (event) {
        ...
        if (!panel || panel.classList.contains("dsh-tb-hidden")) return
